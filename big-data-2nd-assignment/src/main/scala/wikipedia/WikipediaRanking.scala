...
object WikipediaRanking extends WikipediaRankingInterface:
  ...
  val conf: SparkConf = new SparkConf().setAppName("MySparkApp").setMaster("local[*]")
  ...
  def occurrencesOfLang(lang: String, rdd: RDD[WikipediaArticle]): Int = {
      rdd.aggregate(0)((count, article) => if (article.mentionsLanguage(lang)) count + 1 else count, (sum1, sum2) => sum1 + sum2)
  }
  ...
  def rankLangsReduceByKey(langs: List[String], rdd: RDD[WikipediaArticle]): List[(String, Int)] = {
      //makeIndex(langs, rdd).flatMapValues(_.size).reduceByKey(_ + _).sortBy(-_._2).collect().toList
      rdd.flatMap(article => langs.filter(lang => article.mentionsLanguage(lang)).map(lang => (lang,1))).reduceByKey(_ + _).sortBy(-_._2).collect().toList
  }
