...
class StackOverflow extends StackOverflowInterface with Serializable:
  ...
  def scoredPostings(grouped: RDD[(QID, Iterable[(Question, Answer)])]): RDD[(Question, HighScore)] =
    ...
    // grouped.flatMap((qid, postsPair) => (postsPair._1, postsPair._2)).mapValues(_.score).groupByKey
    // val groupedPosts = 
    // grouped.flatMap((qid, postsPair) => (postsPair._1, postsPair._2)).mapValues(answers => answerHighScore(answers))
    // grouped.flatMap{
    //   case (qid, postsPair) => postsPair.map(post => (post._1, post._2))
    // }.reduceByKey{
    //   (answers1, answers2) =>
    //     val allAnswers = answers1 ++ answers2
    //     answerHighScore(allAnswers.toArray)
    // }
    grouped.flatMap( 
      (qid, postPairs) => 
        postPairs.map(postPair => 
          (postPair._1, postPair._2)
        )).groupByKey.mapValues(posts => answerHighScore(posts.toArray))
  ...
  def vectorPostings(scored: RDD[(Question, HighScore)]): RDD[(LangIndex, HighScore)] =
    ...
    // scored.map{ pairScored =>
    //   pairScored.map((question, score) => (firstLangInTag(question.tags), score))
    // }
    scored.flatMap{ 
      case (question, score) =>
  ...
  @tailrec final def kmeans(means: Array[(Int, Int)], vectors: RDD[(Int, Int)], iter: Int = 1, debug: Boolean = false): Array[(Int, Int)] =
    ...
    val clusters = vectors.map{
      vector => 
        val closestMean = findClosest(vector, means)
        (closestMean, vector)
    }
    val newMeansMap: scala.collection.Map[(Int, Int), (Int, Int)] = clusters.groupByKey.mapValues(averageVectors).collectAsMap()
  ...
  def clusterResults(means: Array[(Int, Int)], vectors: RDD[(LangIndex, HighScore)]): Array[(String, Double, Int, Int)] =
    ...
    val median = closestGrouped.mapValues { vs =>
      val langTags = vs.toList.map(_._1)
      val langLabel: String   = langs(langTags.groupBy(identity).mapValues(_.size).maxBy(_._2)._1 / langSpread)  // most common language in the cluster
      val langPercent: Double = (langTags.groupBy(identity).mapValues(_.size).maxBy(_._2)._2 / vs.toList.size.toDouble) * 100 // percent of the questions in the most common language
      val clusterSize: Int    = vs.toList.size
      val scores = vs.map(_._2).toSeq.sorted
      val medianScore: Int    = if (scores.isEmpty) 0 else {
